...
bool break_tie(vector<double> p1,vector<double> p2,int dim){
    ...
    return false;
}double delta(Node* node,vector<double> point){return 0;}
...
double delta(Node* node, vector<double> point, int dim){
    double distn = 0;
    double magn = 0;
    for(int i=0;i<dim;i++){
...
void best_first_algo(Node* root,int n,int dim,vector<double> query_point,int k, ofstream& outfile){
    ...
    while(!candidates.empty()){
        ...
        if(o2.root->left!=NULL){
            candidates.push(*(new object2(o2.root->left,delta(o2.root->left,query_point))));
        }
        
        if(o2.root->right!=NULL){
            candidates.push(*(new object2(o2.root->right,delta(o2.root->right,query_point))));
        }
        
    }
    
    
    vector<object > lastPoints;
    
    vector<object> answers_pts(answer_set.size());
    while(!answer_set.empty()){
        object o = answer_set.top();
//        cout<<o.dist<<" ";
        answers_pts[answer_set.size()-1] = o;
        answer_set.pop();
        //                cout<<endl;
    }
    ...
    int start,end;
    start=end=0;
    double distt = INT_MAX;
    
    for(int i=0;i<answers_pts.size();i++){
        if(answers_pts[i].dist == distt){
            end++;
        }
        else{
            if((end-start)>0){
                sort(answers_pts.begin()+start, answers_pts.begin()+end+1, comparator_ties);
            }
            start=end=i;
            distt = answers_pts[i].dist;
        }
    }
    
    //last points
    if((end-start)>0 || !lastPoints.empty()){
        
        int len = (end==start)?1:(end-start+1);
        for(int i=0;i<len;i++){
            lastPoints.push_back(answers_pts[start+i]);
        }
        
        sort(lastPoints.begin(), lastPoints.end(), comparator_ties);
        
        for(int i=0;i<len;i++){
            answers_pts[start+i]=lastPoints[i];
        }
    }
...
int main(int argc, char* argv[]){
    ...
    for(int i=0;i<n_q;i++){
        best_first_algo(root, n, d_q, query_points[i], k, outfile);
    }
